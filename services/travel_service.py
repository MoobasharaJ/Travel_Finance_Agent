...
class TravelService:
    ...
    def add_pre_trip_expense(
        ...
        total_pre_trip = self.db.get_total_pre_trip_expense(
            trip_id
        )
    ...
    def get_dashboard_data(self, trip_id):
        ...
        trip_progress = calculate_trip_progress(
            days_elapsed,
            trip["duration"]
        )
        ...
        travel_budget_forex = self.currency_service.convert_currency(
            trip["travel_budget"],
            live_rate["exchange_rate"]
        )

        remaining_budget_forex = self.currency_service.convert_currency(
            remaining_budget,
            live_rate["exchange_rate"]
        )
        ...
        trend_90 = self.currency_service.analyze_trend(
            self.currency_service.get_historical_rates(
                trip["currency"],
                90
            )
